...
async function main() {
  ...
  let vertexPath = 'quad.vert';
  let fragmentPath = 'quad.frag';

  let vertexRes = await fetch(vertexPath);
  let vertexSource = await vertexRes.text();
  ...
  let fragmentRes = await fetch(fragmentPath);
  let fragmentSource = await fragmentRes.text();
